...
def is_session_sso(token: str) -> bool:
    """Heuristic: real session cookies are short JWTs with session claims, not wrappers."""
    token = (token or "").strip()
...
def materialize_sso_via_browser(
    page: Any, wrapper_or_sso: str, log=None, timeout: float = 45.0
) -> str:
...
def inject_sso_into_page(page: Any, sso: str, sso_rw: str = "") -> bool:
    """Inject session SSO cookies into the live browser for PKCE / convert."""
    if not page or not sso:
        return False
    rw = sso_rw or sso
    try:
        page.run_js(
            """
const sso = String(arguments[0] || '');
const rw = String(arguments[1] || sso);
if (!sso) return false;
document.cookie = 'sso=' + sso + '; path=/; domain=.x.ai; Secure; SameSite=Lax';
document.cookie = 'sso-rw=' + rw + '; path=/; domain=.x.ai; Secure; SameSite=Lax';
return true;
            """,
            sso,
            rw,
