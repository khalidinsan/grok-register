...
# Verified CSR hash from signup bundle (emailValidationCode chunk). Fragile — may rotate.
KNOWN_NEXT_ACTION = "7f50061dd2f5b389a530e4a048d5fdf0c48d1d9259"
...
class AuthManagementClient:
    ...
    def _rpc(self, method: str, body: bytes, timeout: int = 30) -> dict[str, Any]:
        url = f"{BASE}/{method}"
    ...
    def create_email_validation_code(
        self, email: str, castle_token: str, timeout: int = 30
    ):
    ...
    def create_user_via_server_action(
        ...
        *,
        ...
        resp = self.session.post_raw(
            SIGNUP_URL, body, headers=headers, timeout=timeout
        )
        cookies = self.session.cookies_dict()
        text = ""
        try:
